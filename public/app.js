...
function syncConditionalInputs() {
  const benchmarkFixedRow = $("#benchmarkFixedRow");
  const lprFixedRow = $("#lprFixedRow");
  const benchmarkFloatUnit = $("#benchmarkFloatInput")?.closest(".input-unit");
  const lprBpUnit = $("#lprBpInput")?.closest(".input-unit");
  const lprFloatUnit = $("#lprFloatInput")?.closest(".input-unit");

  if (benchmarkFixedRow) benchmarkFixedRow.hidden = selectedValue("benchmarkMode") !== "fixed";
  if (lprFixedRow) lprFixedRow.hidden = selectedValue("lprMode") !== "fixed";
  if (benchmarkFloatUnit) benchmarkFloatUnit.hidden = selectedValue("benchmarkFloatMode") !== "use";
  if (lprBpUnit) lprBpUnit.hidden = selectedValue("lprBpMode") !== "use";
  if (lprFloatUnit) lprFloatUnit.hidden = selectedValue("lprFloatMode") !== "use";
}
...
async function refreshLpr() {
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(result.message || "刷新接口请求失败");
    }
...
$("#interestCalculator").addEventListener("submit", (event) => {
  ...
  try {
    if (!activeDataset) throw new Error("利率数据尚未加载完成，请刷新页面后再试");
